...
### 标准准确的说法
- `std::unique_ptr<T>` 原理：
- 独占所有权模型，同一时刻只能有一个拥有者负责释放资源。
- 拷贝构造/拷贝赋值被禁用（不可拷贝），避免“双重释放”。
- 支持移动构造/移动赋值（可移动），可把所有权从 A 转移给 B。
- 默认删除器是 `delete`，也可自定义删除器管理文件句柄、socket 等非内存资源。
- `std::shared_ptr<T>` 原理：
- 共享所有权模型，多个 `shared_ptr` 可以共同拥有同一对象。
- 依赖“控制块（control block）”管理元信息，通常包括：
- 强引用计数（strong count）：当前有多少 `shared_ptr` 在拥有对象。
- 弱引用计数（weak count）：当前有多少 `weak_ptr` 在观察对象。
- 删除器/分配器等资源销毁策略。
- 强计数归零时销毁对象本体；弱计数也归零后控制块才释放。
- `std::weak_ptr<T>` 原理：
- 弱引用，不拥有对象，不增加强计数，仅观察对象是否还活着。
- 通过 `lock()` 尝试提升为 `shared_ptr`：成功表示对象仍存在，失败得到空指针。
- 典型用途是打破 `shared_ptr` 循环引用。
- 三者核心区别（面试高频）：
- 所有权：`unique_ptr` 独占，`shared_ptr` 共享，`weak_ptr` 观察。
- 生命周期影响：`unique_ptr/shared_ptr` 影响对象生死，`weak_ptr` 不影响。
- 成本：`unique_ptr` 最轻；`shared_ptr/weak_ptr` 因控制块和计数维护更重。
- `make_shared` 机制补充：
- `std::make_shared<T>(...)` 通常可把“对象 + 控制块”一次分配，局部性更好、分配次数更少。
- 直接 `shared_ptr<T>(new T(...))` 往往是两次分配（对象一次、控制块一次）。
...
- `shared_ptr`：多人共同持有。
- `weak_ptr`：只观察，不算“持有者”。
- `shared_ptr` 背后有个“登记处”（控制块）在记人数：强计数清零对象才会真正销毁。
- 循环引用本质是“大家互相把对方强计数托住”，所以要用 `weak_ptr` 在环上断开一条强拥有关系。

### 面试回答简版模板
`智能指针按所有权分三类：unique_ptr 独占且可移动，shared_ptr 共享并由控制块维护强/弱计数，weak_ptr 只观察不持有。工程上默认优先 unique_ptr；确需共享再用 shared_ptr；遇到环用 weak_ptr 断强引用。`
...
// 反例：p 仍是旧地址（悬空指针），继续用会是未定义行为
// std::cout << *p << "\n"; // use-after-free
// delete p;                  // double free 风险

// 正例：释放后立刻置空，让失效状态显式化
p = nullptr;
if (p == nullptr) {
    std::cout << "pointer is invalid now" << "\n";
}
```

- 说明：`delete` 只释放“p 指向的对象”，不会自动把 `p` 变量改成空。
- 说明：把指针置空不能修复所有权设计问题，但能显著减少悬空指针被误用。
